...
def activate_drone_detection(drone, color=0):
    ...
    drone.configure("general:navdata_demo","FALSE")
...
# Autonomous Flight
def reactivate_navigation_control(drone):
    "Reactivate possibility to send command if drone was previoutly in autonomous mode"
    return drone.unlock_command()
...
def goto_gps_point(drone, longitude, latitude, altitude=3, cap=0):
    ...
    param1 = "8,2000000,"+str(longi)+","+str(lati)+","+str(alt)+",0,0,0," + str(cap) + ",0"
    # Let's go !
    drone.lock_command() # Prevent drone from sending further commands
    ...
    drone.configure("control:travelling_enable","TRUE")
